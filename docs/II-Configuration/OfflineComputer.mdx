...
Cardano uses numerous cryptographic key pairs to secure your wallet and stake pool. A key pair is comprised of a public verification key file having the `.vkey` file name extension and a secret key file having an `.skey` extension. You use key pairs to generate addresses, certificates and other related files securely. For more details, see [Cardano Key Pairs](https://developers.cardano.org/docs/operate-a-stake-pool/cardano-key-pairs/).
...
For convenience when following the CoinCashew guide, create a `NODE_HOME` environment variable on your air-gapped, offline computer set to the same file path that you set on your block-producing and relay nodes when [Installing Shared Libraries](/SharedLibraries).
...
5\. Using removable media, copy the following files from a block-producing or relay node where you installed `libsecp256k1` to the same location on your air-gapped, offline computer:
```bash
...
/usr/local/include/secp256k1_ellswift.h
/usr/local/include/secp256k1_extrakeys.h
/usr/local/include/secp256k1.h
/usr/local/include/secp256k1_musig.h
/usr/local/include/secp256k1_preallocated.h
/usr/local/include/secp256k1_schnorrsig.h
```
...
sudo chown root:root secp256k1*
sudo chmod 644 secp256k1*
```
