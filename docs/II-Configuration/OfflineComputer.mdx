...
6\. To verify that the IOHK Sodium shared library is recognized, type <code>ldconfig -p | grep libsodium</code> and then confirm that the output includes the following lines:
```
