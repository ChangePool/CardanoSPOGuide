...
:::info
You configured the `topology.json` file when [Configuring Topology](/ConfiguringTopology). You downloaded the `config-bp.json` and `config.json` files when [Downloading Configuration Files](/ConfigurationFiles).
:::
