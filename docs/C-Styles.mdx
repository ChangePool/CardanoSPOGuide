...
<!-- To create a comment in Markdown (MD), use HTML comments -->

<!-- The HTMLContent function references the CodeSample constant, preserving whitespace in the multiline string -->
export const CodeSample = `function Code()
{
  Sample Code;
  Sample Code;
  Sample Code;
...
export function HTMLContent() {
  /* To create a comment in JavaScript XML (JSX) outside elements, use JavaScript comments */
  return (
    <div>
      {/* To create a comment inside a JSX element, wrap a JavaScript multiline comment using braces */}
      ...
      <p>To preserve whitespace in a code sample, export a multiline string using backticks (<code>`</code>), and then reference the exported variable:</p>
      <pre>
        {/* The CodeSample constant is defined at the beginning of the current file */}
        {CodeSample} 
      </pre>
      <table class="box">
        <tbody>
          <tr>
            <td width="125pt"><p class="boxLabel">Note</p></td>
            <td><p class="boxText">For more details on displaying code samples, see the source file for the current topic available at the relative path <code>/docs/C&#8209;Styles.mdx</code></p></td>
          </tr>
        </tbody>
      </table>
