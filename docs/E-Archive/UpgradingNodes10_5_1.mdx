...
:::info
If you follow the Coin Cashew instructions for [Downloading Configuration Files](/ConfigFiles), then `<ConfigurationFileFolder>` is `$NODE_HOME` If needed, you can also use the environment variable `$NODE_CONFIG` to indicate the `mainnet` cluster in configuration file names.
:::
