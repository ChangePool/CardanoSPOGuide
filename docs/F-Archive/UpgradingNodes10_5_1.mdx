---
title: "Upgrading to Cardano Node 10.5.1"
sidebar_label: "Upgrading to Cardano Node 10.5.1"
slug: /UpgradingNodes10_5_1
hide_title: false
...
Technical writing by [Change Pool (ticker CHG)](https://coincashew.io)
:::
...
Complete the following procedures on each block producer and relay node comprising your stake pool as needed to complete the upgrade.
...
:::info
If you follow the Coin Cashew instructions for [Starting the Nodes](/StartingNodes), then you can type `$NODE_HOME` to replace `<gLiveViewFolder>`
:::
...
:::info
For details on setting the `NODE_HOME` environment variable, see the topic [Installing Shared Libraries](/SharedLibraries)
:::
...
:::info
If you follow the Coin Cashew instructions for [Automating Cardano Node Startup](/AutomatingStartup), then `<CardanoServiceName>` is `cardano-node`
:::
...
mv config-bp.json config-bp.bak
...
:::info
If you follow the Coin Cashew instructions for [Downloading Configuration Files](/ConfigurationFiles), then `<ConfigurationFileFolder>` is `$NODE_HOME` If needed, you can also use the environment variable `$NODE_CONFIG` to indicate the `mainnet` environment in configuration file names.
:::
...
4\. On the Cardano Configurations page, click the following links to download configuration files for the `mainnet` environment to the folder where you created backups of your current configuration files in step 2: `config*`, `byronGenesis`, `shelleyGenesis`, `alonzoGenesis`, `conwayGenesis` and `topology`
...
5\. On your air-gapped, offline computer, ensure that [libsecp256k1](/OfflineComputer#Installing-libsecp256k1) and [blst](/OfflineComputer#Installing-the-blst-Library) are installed and up to date.
...
:::tip
Congratulations on completing the update! :sparkles:
...
Any feedback and all pull requests are much appreciated.
:::
