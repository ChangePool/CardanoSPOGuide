...
export function HTMLContent() {
  ...
  return (
    <div>
      ...
      <p>Registering your stake pool to join the Cardano network involves preparing a stake pool registration certificate, and then submitting the certificate to the blockchain.</p>
...
...`}
  ...
  --out-file $NODE_HOME/deleg.cert`}
      ...
      <p>The following procedure explains how to submit a stake pool registration certificate to the Cardano blockchain ledger in the Mainnet environment. If you are registering a stake pool in a Testnet environment, then replace each instance of the <code>--mainnet</code> option with <code>--testnet-magic &lt;MagicNumber&gt;</code> where <code>&lt;MagicNumber&gt;</code> is the network magic number for the Testnet that you use.</p>

      <p class="pi">To submit a stake pool registration certificate:</p>
