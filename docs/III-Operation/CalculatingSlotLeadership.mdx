...
export function HTMLContent() {
  return (
    <div>
      ...
      <p>As a stake pool operator, you aim to keep the stake pool that you operate online at all times. However, knowing in advance when the stake pool is elected to produce a block allows you to schedule required maintenance, for example, without missing an opportunity to prepare a block.</p>

      <p>Using Cardano CLI, you can query the slots and Coordinated Universal Times (UTCs) when a stake pool that you operate is elected to produce a block in the current epoch. Within 36 hours of the next epoch starting, you can also query the slots and times when the stake pool is elected to produce blocks in the next epoch.</p>
