...
export function HTMLContent() {
  ...
  --next`}
    ...
    },
    {
      ...
      <p>The following script uses the <code>slotInEpoch</code> EKG metric to test if the current epoch is more than 70 percent complete. If needed, the script waits until the leadership schedule for the next epoch is available. When the leadership schedule is available, the script queries the slot leadership schedule to retrieve the slot numbers and times when the stake pool you operate is scheduled to produce blocks in the next epoch, and then saves the data in a new text file using JSON format.</p>
