...
|     Release Date     | Cardano Node Version | Cardano CLI Version | Cardano Tracer Version | GHC Version | Cabal Version |
|  :----------------:  | :------------------: | :-----------------: | :--------------------: | :---------: | :-----------: |
|      {nodeReleaseDate}      |        {nodeVersionNumber}        |      {cliVersionNumber}      |         {tracerVersionNumber}          |    {ghcVersionNumber}    |    {cabalVersionNumber}   |
...
1. To stop an instance of Cardano Node, type the following command using a terminal window where `<CardanoServiceName>` is the name of the systemd service running Cardano Node:
...
mv tracer-config.json tracer-config.bak
...
5\. If you are upgrading the Cardano Node instance running on the block-producing node in your stake pool configuration, then type the following commands using a terminal window on the block producer to rename the node configuration file:
```bash
cd ${NODE_HOME}
mv config.json config-bp.json
```

6\. Using [`diff`](https://www.man7.org/linux/man-pages/man1/diff.1.html) or a similar file comparison utility, identify and copy customizations as needed from the `config*.bak`, `tracer-config.bak` and `topology.bak` configuration file backups that you created in step 2 to the respective configuration file that you downloaded in step 4.
...
5\. To install the new node, cli and tracer binaries, type:
...
```
sudo mv ~/tmp2/bin/cardano-tracer /usr/local/bin/
```
...
2\. To build Cardano Node binaries using the source code that you downloaded in step 1, type the following commands where `<NewFolderName>` is the name of the folder you created in step 1:
...
# Install new packages required for Cardano Node 10.7.1 and above
sudo apt-get install libsnappy-dev protobuf-compiler liburing-dev -y
...
cabal configure -O0 -w ghc-`}
    {ghcVersionNumber}
    {`
# Compile the cardano-node, cardano-cli and cardano-tracer packages found in the current directory
cabal build all
...
The time required to compile the `cardano-node`, `cardano-cli` and `cardano-tracer` packages may be a few minutes to hours, depending on the specifications of your computer.

3\. When the compiler finishes, to verify the version numbers of the new `cardano-node`, `cardano-cli` and `cardano-tracer` binaries type:

```bash
$(./scripts/bin-path.sh cardano-node) --version
$(./scripts/bin-path.sh cardano-cli) --version
$(./scripts/bin-path.sh cardano-tracer) --version
```
...
2\. To replace the existing `cardano-node`, `cardano-cli` and `cardano-tracer` binaries, type the following commands where `<DestinationPath>` is the absolute file path to the folder where you install Cardano Node binaries on your local computer:
...
sudo cp -p "$(./scripts/bin-path.sh cardano-tracer)" <DestinationPath>/cardano-tracer
```
...
### Implementing Cardano Tracer

Cardano Tracer provides flexible and efficient monitoring of Cardano Node instances in your stake pool configuration using a modular configuration, ensuring that Cardano Node focuses on core operations while Tracer manages logging, monitoring and external communication.

By default, Cardano Node 10.6.2 and above enable Cardano Tracer.

<table class="box">
  <tbody>
    <tr>
      <td class="boxLabel"><p class="boxLabel">Important<br /><img src={useBaseUrl('img/BoxImportant.png')} alt="Important" /></p></td>
      <td class="boxText"><p class="boxText">The old tracing mechanism is deprecated. Implementing Cardano Tracer is required.</p></td>
    </tr>
  </tbody>
</table>

**To implement Cardano Tracer:**

1\. On the computer hosting the block-producing node in your stake pool configuration, follow the procedure for [Running Cardano Tracer Automatically](/RunningTracer).

2\. In the `$NODE_HOME/startCardanoNode.sh` file on the block-producing node, add the `--tracer-socket-path-connect` option to the `cardano-node run` command. For more details, refer to the topic [Automating Cardano Node Startup](/AutomatingStartup).

3\. To open the `cardano-node.service` file on the block-producing node using a text editor, type:

```bash
sudo nano /etc/systemd/system/cardano-node.service
```

4\. In the `cardano-node.service` file, edit the following lines in the `[Unit]` block:

```bash
Wants = network-online.target cardano-tracer.service
After = network-online.target cardano-tracer.service
```

4\. To save and close the `cardano-node.service` file, press `CTRL+X` and then follow the prompts.

5\. To apply the configuration changes, type:

```bash
sudo systemctl daemon-reload
```

6\. Repeat steps 1 to 5 on each computer hosting a relay node in your stake pool configuration.

7\. On each computer in your stake pool configuration, update scripts as needed to use Cardano Tracer endpoints and metric names. For more details, see the topic [Monitoring Metrics](/MonitoringMetrics).

### Completing the Upgrade

**To finish upgrading Cardano Node binaries:**

1\. On the computer hosting the block-producing node in your stake pool configuration, to verify that you installed the new Cardano Node binaries successfully, type:
...
4\. Repeat steps 1 to 3 on each computer hosting a relay node in your stake pool configuration.

5\. Copy the new `cardano-cli` binary to the air-gapped, offline computer that you use to sign transactions for your stake pool.
...
Upgrading to a new Cardano Node version may require replaying the copy of the blockchain ledger residing on the local computer. Here is an example of Cardano Node log entries showing progress when replaying the blockchain ledger:

```bash
...

[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3368857 out of 104785521. Progress: 3.22%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3389817 out of 104785521. Progress: 3.24%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3410780 out of 104785521. Progress: 3.26%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3421288 out of 104785521. Progress: 3.27%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3442207 out of 104785521. Progress: 3.29%

...

[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3609875 out of 104785521. Progress: 3.45%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3630832 out of 104785521. Progress: 3.47%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3651780 out of 104785521. Progress: 3.49%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3672859 out of 104785521. Progress: 3.51%
[ChainDB.ReplayBlock.LedgerReplay](Info,5) Replayed block: slot 3683233 out of 104785521. Progress: 3.52%

...
```

The task of replaying the ledger may require hours to complete.

<!-- For details on monitoring the Cardano Node log, see the section [Verifying the Upgrade](#Verifying-the-Upgrade). -->

## Verifying the Upgrade

To verify that the upgrade is successful, on each computer hosting an instance of Cardano Node in your stake pool configuration open a dashboard, or type the following command using a terminal window to follow the Cardano Node log:
...
:::info
For more details, see the section [Monitoring the Cardano Node Log](/StartingNodes#Monitoring-the-Cardano-Node-Log).
:::
