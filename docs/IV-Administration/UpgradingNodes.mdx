...
1\. Follow the procedure for [Running Cardano Tracer Automatically](/RunningTracer).

2\. In the `$NODE_HOME/config*.json` file for each computer in your stake pool configuration, add the `--tracer-socket-path-connect` option to the `cardano-node run` command. For more details, refer to the topic [Automating Cardano Node Startup](/AutomatingStartup).

3\. Update scripts in your stake pool configuration to use Cardano Tracer endpoints and metric names. For more details, see the topic [Monitoring Metrics](/MonitoringMetrics).
...
To verify that the upgrade is successful, open a dashboard or type the following command using a terminal window to follow the Cardano Node log:
