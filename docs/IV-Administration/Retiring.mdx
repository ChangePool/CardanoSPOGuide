...
echo "Currently, the earliest a stake pool can retire is at the end of epoch $\{minRetirementEpoch\}"
echo "and the latest a stake pool can retire is at the end of epoch $\{maxRetirementEpoch\}"`}
          </CodeBlock>
        </li>
...
echo "Change: $\{txOut\}"`}
  ...
  --out-file $NODE_HOME/tx.raw`}
      ...
      <ol>
        <li>Wait until the epoch to retire that you specified using the stake pool deregistration certificate ends.</li>
