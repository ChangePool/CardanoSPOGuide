---
title: "Implementing and Using Secure Shell"
sidebar_label: "Implementing and Using Secure Shell"
slug: /Hardening/SSH
hide_title: false
...
1\. On the computer that you want to access remotely, follow the [How to Install and Enable SSH on Ubuntu 26.04](https://linuxconfig.org/how-to-enable-ssh-on-ubuntu-26-04) tutorial.
...
6\. To change the port number where the SSH Server listens, type the following command in the remote terminal window:
```bash
sudo mkdir -p /etc/systemd/system/ssh.socket.d
sudo nano /etc/systemd/system/ssh.socket.d/listen.conf
```

7\. In the `listen.conf` file, add the following lines where `<IPAddress>` and `<PortNumber>` are the IP address and port number, respectively, where the SSH Server listens:
```bash
[Socket]
ListenStream=
ListenStream=<IPAddress>:<PortNumber>
```

:::note
To check port numbers for potential conflicts, refer to the [List of TCP and UDP Port Numbers](https://en.wikipedia.org/wiki/List_of_TCP_and_UDP_port_numbers).
:::

8\. Save and close the `listen.conf` file.

9\. To edit additional SSH Server settings, type the following command in the remote terminal window:
```bash
sudo nano /etc/ssh/sshd_config.d/01-hardening.conf
```

10\. Add the following lines in the `01-hardening.conf` file, where `<PortNumber>` is a port number between 1024 and 49151 that is unused on the computer hosting the SSH Server:
```bash
...
11\. Save and close the `01-hardening.conf` file.

12\. To validate the syntax of SSH Server settings, type:
```bash
sudo sshd -t
```

13\. If validating settings returns no errors, then type the following command to restart the SSH Server:
```bash
sudo systemctl restart ssh.socket
```

14\. To verify that you can connect to the SSH Server using public key authentication, type the following command on the computer connecting to the SSH Server as a client where `<SSHServer>` is the IP address or domain name where the SSH Server listens and `<PortNumber>` is the port number that you set in step 7:
```bash
...
<p class="pi">To transfer files from your local computer to a remote computer:</p>

<ul class="ss">
  <li>Using a terminal window on the local computer, type the following command where `<PortNumber>` is the port where the SSH Server on the remote computer listens; `<LocalPath>` is the path to the file(s) on the local computer to copy; `<SSHServer>` is the IP address or domain name where the remote SSH Server listens; and, `<RemotePath>` is the destination file path on the remote computer:</li>
...
:::warning
If you connect remotely to a computer hosting SSH Server in order to configure two-factor authentication, then you may not regain remote access if you make an error in the configuration.
:::
...
5\. To edit SSH Server settings, type:
```bash
sudo nano /etc/ssh/sshd_config.d/01-hardening.conf
```

6\. In the `01-hardening.conf` file, add the following lines:
```bash
UsePAM yes
...
7\. Save and close the `01-hardening.conf` file.
...
1\. On a client computer where you implemented key authentication and an SSH `config` file, open a terminal window, and then type the following command:
```bash
ssh cardano-server
```

2\. Follow the prompts on screen to complete two-factor authentication.
...
The `/etc/ssh/sshd-config.d` directory contains configuration files processed in alphabetical order by file name via the `Include /etc/ssh/sshd_config.d/*.conf` statement found as the first line in the `/etc/ssh/sshd-config` file. If a configuration file found in the `sshd-config.d` folder sets an option also set in the `sshd-config` file, then the option set in the `sshd-config` file may not apply. SSH Server applies the last definition of a configuration option.
...
2\. Verify that the SSH Server applies the following options, where `<PortNumber>` is the port number that you set when [Configuring Secure Shell](#Configuring-Secure-Shell):
```bash
Port <PortNumber>
PermitRootLogin no
PubkeyAuthentication yes
...
3\. If the SSH Server does NOT apply one or more of the options listed in step 2, then examine the contents of the configuration files found in the `/etc/ssh/sshd-config.d` directory to identify the configuration file(s) setting the same option(s) after the `01-hardening.conf` file sets the options. For example, if a configuration file named `50-cloud-init.conf` sets `PasswordAuthentication yes` then type the following commands to rename the file:
```bash
cd /etc/ssh/sshd-config.d
sudo mv ./50-cloud-init.conf ./50-cloud-init.conf.disabled
```

4\. To restart the SSH Server, type:
```bash
sudo systemctl restart ssh.socket
```

5\. If you edited configuration options in step 3, then repeat steps 1 to 4 as needed until the options that the SSH Server applies are correct.
