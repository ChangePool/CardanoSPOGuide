...
sidebar_label: "Setting Up a Firewall"
...
To protect the relay nodes in your stake pool configuration against Denial of Service (DOS) attacks, you can create an `iptables` rule on each relay that limits the number of connections from the same IP address to the port where Cardano Node listens.
...
On computers in your stake pool configuration, you need to configure UFW to allow incoming connections to the following applications:
...
On the block-producing node, type the following command where `<IPAddress>` is the IP address of a relay node in your stake pool configuration, and `<PortNumber>` is the port where the local instance of Cardano Node listens:
```
...
Listing all IP addresses connected to a specific port on a computer may also be useful.

**To list all current connections to a port on the local computer:**

1\. Using a terminal window, type the following command where `<PortNumber>` is the port number for which you want to list connections:
```
