...
2\. Type the following command to navigate to the working directory that you created in the procedure [Installing GHC and Cabal](/GHC):
```bash
cd $HOME/git
```
...
8\. To confirm that the version installed on your computer matches the latest release available in the Cardano Node [GitHub repository](https://github.com/IntersectMBO/cardano-node), type:
```bash
cardano-node --version
cardano-cli --version
```

9\. On each computer hosting a relay or block-producing node in your stake pool configuration, repeat steps 1 to 8
