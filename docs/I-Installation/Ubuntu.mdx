...
:::warning
Ensure that measures you implement to secure shared memory are compatible with applications running on the computer. For example, Web browsers may require read and write access to shared memory for rendering and caching.
:::
...
2\. At the end of the `fstab` file, add the following line to mount the shared memory filesystem securely using read-only access:
```
...
To mount the shared memory filesystem securely using read-write access, add the following line:
```
