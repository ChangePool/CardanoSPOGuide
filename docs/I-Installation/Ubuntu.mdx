...
- To [Configure Secure Shell](#configuring-secure-shell) connections:
    ...
    - [Use Port Forwarding](#using-port-forwarding)
...
- [Validate Network Connections](#telescope-validating-network-connections) on the computer regularly.
...
For example, if a computer that you access remotely via SSH operates a server having an HTTP endpoint on port 3000, then the following command allows you to access the endpoint by visiting `http://localhost:3010` using your Web browser on the local computer:
```
...
## :telescope: Validating Network Connections
