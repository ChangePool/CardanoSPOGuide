...
For each Cardano Node release, Input-Output recommends compiling binaries using specific versions of GHC and Cabal. For example, refer to the [Releases](https://github.com/IntersectMBO/cardano-node/releases) page in the Cardano Node GitHub repository to determine the GHC and Cabal versions required for the current Cardano Node release. _Table 1_ lists GHC and Cabal versions that the *How to Set Up a Cardano Stake Pool* guide currently supports.
