...
export function HTMLContent() {
  return (
    <div>
      ...
      {/* <!-- Test whether the hardware wallet signing file named hw-stake.hwsfile that cardano-hw-cli generates may sign a transaction to send funds from the corresponding payment address. If so, then the procedure to add a hardware wallet owner to a stake pool is NOT secure. If the procedure is not secure, then remove the Securing a Stake Pool Using a Hardware Wallet topic from the current version of the CoinCashew guide. Consider adding an IMPORTANT note at the top of the topic in an older publication of the CoinCashew guide explaining the additional risks associated with using cardano-hw-cli
      ...
      <p>When <a href={useBaseUrl('/UpdatingPoolRegCert')}>Updating the Stake Pool Registration Certificate</a>, all stake pool owners must witness the transaction using the respective stake address signing file.</p>
