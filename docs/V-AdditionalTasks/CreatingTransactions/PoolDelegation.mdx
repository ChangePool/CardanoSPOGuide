...
export function HTMLContent() {
  ...
  return (
    <div>
      ...
      <p><a href={useBaseUrl('/RegisteringPool')}>Registering a Stake Pool</a> involves submitting a stake delegation certificate to the Cardano blockchain ledger. However, using Cardano CLI you can also delegate a wallet address to an existing stake pool without registering a stake pool.</p>
      ...
      <ol>
        <li>If you need to create a new wallet address for delegating to an existing stake pool, then follow the procedures for <a href={useBaseUrl('/CreatingWallet')}>Creating a Wallet</a>, and then <a href={useBaseUrl('/CreatingWallet#FundingAddress')}>Funding Your Payment Address</a>.
          <table class="box">
            <tbody>
              <tr>
        ...
        <li>If you are delegating the wallet address to a stake pool for the first time, then follow the procedure for <a href={useBaseUrl('/RegisteringAddress')}>Registering a Stake Address</a>.</li>
