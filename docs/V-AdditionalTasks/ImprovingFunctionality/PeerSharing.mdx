...
export function HTMLContent() {
  return (
    <div>
      ...
      <p>To improve network security, the peer sharing feature available in the Cardano Node software allows you to connect one or more relay nodes to the Cardano network without registering the relay(s) on the blockchain. You can use one or more relay nodes that you register on the blockchain to advertise your unregistered relay(s) to other nodes in the network.</p>

      {/* <!-- In Cardano Node 10.6.2 and above, the PeerSharing and related keys in the node configuration file are obsolete. Now, Cardano Node automatically enables peer sharing when Cardano Node runs as a relay node, and disables peer sharing when Cardano Node runs as a block producer. --> */}

      <p class="pi">To implement peer sharing:</p>
...
\}`}
          ...
          <table class="box">
            <tbody>
              <tr>
                <td class="boxLabel"><p class="boxLabel">Note<br /><img src={useBaseUrl('img/BoxNote.png')} alt="Note" /></p></td>
                <td class="boxText"><p class="boxText">For more details on editing the <code>topology.json</code> file, see the topic <a href={useBaseUrl('/ConfiguringTopology')}>Configuring Topology</a>.</p></td>
              </tr>
            </tbody>
          </table>
        </li>
      ...
      <p>After the registered relay node restarts, verify that other relay nodes in the Cardano network gradually begin connecting to the unregistered relay(s) in your stake pool configuration.</p>
