...
mv $\{textfileCollectorPath\}/exportMetrics.tmp $\{textfileCollectorPath\}/exportMetrics.prom`}
        ...
        <li>To verify that custom metrics you may generate export successfully, using a Web browser press <b>CTRL+F5</b> to perform a hard refresh on the Prometheus Web user interface, and then query the custom metrics.</li>
      </ol>
      ...
      <table class="box">
        <tbody>
          <tr>
            ...
            <td class="boxText"><p class="boxText">To create a cron job that schedules running a script exporting custom metrics using a textfile collector regularly, type the following command to create a cron job that runs using root privileges:
              <CodeBlock language="bash">
                {`sudo crontab -e`}
              </CodeBlock></p>
            </td>            
