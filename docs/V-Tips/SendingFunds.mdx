...
export function HTMLContent() {
  return (
    <div>
      ...
      <table class="box">
        <tbody>
          <tr>
            ...
            <td class="boxText"><p class="boxText">If you are registering a stake address in a testnet environment, then replace each instance of the <code>--mainnet</code> option with <code>--testnet-magic &lt;MagicNumber&gt;</code> where <code>&lt;MagicNumber&gt;</code> is the network magic number for the testnet network that you use.</p></td>
          </tr>
        </tbody>
      </table>
      ...
      <ol>
        <li>On the computer hosting your block-producing node, using a terminal window type the following commands to save protocol parameters in a file using JSON format:
          <CodeBlock>
            {saveParameters}
          </CodeBlock>
        </li>
        ...
        <li>To build a temporary transaction for calculating the transaction fee, type:
          <pre>
            ...
            $&#123;tx_in&#125; \<br />
            &nbsp;&nbsp;--tx-out $(cat $NODE_HOME/payment.addr)+$(( $&#123;total_balance&#125; - $&#123;amountToSend&#125; )) \<br />
        ...
        <li>To calculate the fee for the transaction, type:
          <pre>
            fee=$(cardano-cli conway transaction calculate-min-fee --output-json \<br />
            ...
            &nbsp;&nbsp;--protocol-params-file $NODE_HOME/params.json | jq -r '.fee')<br />
            echo fee: $&#123;fee&#125;
          </pre>
        </li>
